...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about an Kava node"
		echo
		...
		echo -e "https://github.com/SecorD0/Kava/blob/main/node_info.sh - script URL"
...
token_name="kava"
node_dir="$HOME/.kava/"
wallet_name="$kava_wallet_name"
wallet_address="$kava_wallet_address"
wallet_address_variable="kava_wallet_address"
validator_address="$kava_validator_address"
validator_address_variable="kava_validator_address"
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/Kava/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
